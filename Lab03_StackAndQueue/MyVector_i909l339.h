...
class MyVector
{
  ...
  public:
    ...
    MyVector(const MyVector &rhs) : 
        ...
        data { nullptr}
    {
        // allocate |theCapacity| for items in vector
        data = new DataType[theCapacity];
        // iterate through each item in rhs to create the same list for lhs
    ...
    void reserve(size_t newCapacity)
    {
        ...
        DataType *newVector = new DataType[newCapacity];
        for (size_t i = 0; i < theSize; ++i)
            newVector[i] = std::move(data[i]);
            
        theCapacity = newCapacity;
        std::swap(data, newVector);
        delete[] newVector;
    }
